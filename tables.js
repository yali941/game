...
export function createTableService({ roll = () => randomInt(1, 7), records } = {}) {
  const rooms = new Map();
  const connected = p => Boolean(p && !p.gone && p.streams.size);
  const snapshot = (r, p) => ({ code: r.code, kind: r.kind, count: r.count, yourSeat: p.seat, host: 1, game: r.game, started: r.started, closed: r.closed, ready: r.ready, round: r.round, version: r.version, seq: r.seq, players: r.players.map(p => p ? { seat: p.seat, connected: connected(p), gone: p.gone } : null) });
  const broadcast = r => {
  ...
  async function handle(req, res, url, data = {}) {
    ...
    if (action === 'create') {
      ...
      p.profileId = records?.lookup(req)?.id;
    ...
    if (action === 'join') {
      ...
      const p = seat(index + 1); p.profileId = records?.lookup(req)?.id; r.players[index] = p; broadcast(r);
    ...
    if (!r.players.every(connected)) throw new Error('等待所有棋手连接后继续');
    if (action === 'reaction') {
      if (!['poop', 'heart', 'bomb'].includes(data.kind)) throw new Error('不支持的互动');
    ...
    if (action === 'start') {
      ...
      if (r.ready.length === r.count) { r.game = newTableGame(r.kind, r.count); r.round++; r.ready = []; r.game.turn = (r.round - 1) % r.count + 1; }
